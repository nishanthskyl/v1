...
// PRG pattern: Check for success message from session
if (isset($_SESSION['form_success_message'])) {
    $success = $_SESSION['form_success_message'];
    unset($_SESSION['form_success_message']);
}
// Similarly, you could check for an error message if you were to redirect on error
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['upload_wall_of_love_image'])) {
        ...
        if (isset($_FILES['images']) && !empty($_FILES['images']['name'][0])) {
            ...
            // Process the main image (first file)
            $mainImageBasename = basename($_FILES["images"]["name"][0]);
            // Sanitize the main image filename
            $safeMainImageName = preg_replace("/[^a-zA-Z0-9._-]/", "_", $mainImageBasename);
            if (empty($safeMainImageName) || $safeMainImageName === '.' || $safeMainImageName === '..') {
                $error = "Invalid main image filename after sanitization.";
                $uploadOk = 0;
            } else {
                $mainImageTargetFile = $targetDir . $safeMainImageName;
                $mainImageFileType = strtolower(pathinfo($mainImageTargetFile, PATHINFO_EXTENSION));

                // Check if main image file is a actual image or fake image
                $check = getimagesize($_FILES["images"]["tmp_name"][0]);
                if ($check === false) {
                    $error = "Main file is not an image.";
                    $uploadOk = 0;
                }

                // Check main image file size (9MB max)
                if ($_FILES["images"]["size"][0] > 9000000) {
                    $error = "Sorry, your main image file is too large (max 9MB).";
                    $uploadOk = 0;
                }

                // Allow certain file formats for main image
                if (!in_array($mainImageFileType, ["jpg", "jpeg", "png", "gif", "webp"])) {
                    $error = "Sorry, only JPG, JPEG, PNG, GIF & WEBP files are allowed for the main image.";
                    $uploadOk = 0;
                }

                // Prevent overwriting existing files by appending a number if necessary
                $counter = 0;
                $originalTargetFile = $mainImageTargetFile;
                $originalSafeImageName = $safeMainImageName;
                while (file_exists($mainImageTargetFile)) {
                    $counter++;
                    $fileNameWithoutExt = pathinfo($originalSafeImageName, PATHINFO_FILENAME);
                    $fileExt = pathinfo($originalSafeImageName, PATHINFO_EXTENSION);
                    $safeMainImageName = $fileNameWithoutExt . "_" . $counter . "." . $fileExt;
                    $mainImageTargetFile = $targetDir . $safeMainImageName;
                }


                if ($uploadOk == 1) {
                    if (move_uploaded_file($_FILES["images"]["tmp_name"][0], $mainImageTargetFile)) {
                        $mainImagePath = $mainImageTargetFile;
                    } else {
                        $error = "Sorry, there was an error uploading your main image.";
                        $uploadOk = 0;
                    }
                }
            }
        } else {
            // No image uploaded, which is not allowed if 'required' is set in HTML, but handle defensively
            $error = "Product image is required.";
            $uploadOk = 0;
        }

        if ($uploadOk == 1 && $mainImagePath !== null) {
            // Insert product
            ...
            $stmt->execute([
                ...
                $_POST['description'],
                $_POST['price'],
                $mainImagePath, // Use the path of the uploaded main image
                $_POST['category_id']
            ]);

            $productId = $pdo->lastInsertId();

            // Create directory for additional product images
            $productImagesDir = "uploads/products/" . $productId . "/";
            if (!is_dir($productImagesDir)) {
                if (!mkdir($productImagesDir, 0777, true)) {
                    $error = "Failed to create product image directory. Please check permissions.";
                    // Potentially roll back product insertion or log this critical error
                    // For now, we'll set an error and the main image is already saved.
                    $uploadOk = 0; // Stop further image processing if dir fails
                }
            }

            // Process additional images (if any)
            if ($uploadOk == 1 && count($_FILES["images"]["tmp_name"]) > 1) {
                for ($i = 1; $i < count($_FILES["images"]["tmp_name"]); $i++) {
                    if (isset($_FILES["images"]["name"][$i]) && $_FILES["images"]["error"][$i] == UPLOAD_ERR_OK) {
                        $additionalImageBasename = basename($_FILES["images"]["name"][$i]);
                        // Sanitize the additional image filename
                        $safeAdditionalImageName = preg_replace("/[^a-zA-Z0-9._-]/", "_", $additionalImageBasename);

                        if (empty($safeAdditionalImageName) || $safeAdditionalImageName === '.' || $safeAdditionalImageName === '..') {
                            // Skip invalid filenames or add to a list of errors
                            error_log("Skipping additional image with invalid filename after sanitization: " . $additionalImageBasename);
                            continue;
                        }

                        $additionalImageTargetFile = $productImagesDir . $safeAdditionalImageName;
                        $additionalImageFileType = strtolower(pathinfo($additionalImageTargetFile, PATHINFO_EXTENSION));

                        $check = getimagesize($_FILES["images"]["tmp_name"][$i]);
                        if ($check === false) {
                            error_log("Skipping additional file (not an image): " . $additionalImageBasename);
                            continue;
                        }

                        if ($_FILES["images"]["size"][$i] > 9000000) { // 9MB max
                            error_log("Skipping additional image (too large): " . $additionalImageBasename);
                            continue;
                        }

                        if (!in_array($additionalImageFileType, ["jpg", "jpeg", "png", "gif", "webp"])) {
                            error_log("Skipping additional image (invalid type): " . $additionalImageBasename);
                            continue;
                        }

                        // Prevent overwriting existing files by appending a number if necessary
                        $counter_add = 0;
                        $originalAddTargetFile = $additionalImageTargetFile;
                        $originalSafeAddImageName = $safeAdditionalImageName;
                        while (file_exists($additionalImageTargetFile)) {
                            $counter_add++;
                            $fileNameWithoutExt_add = pathinfo($originalSafeAddImageName, PATHINFO_FILENAME);
                            $fileExt_add = pathinfo($originalSafeAddImageName, PATHINFO_EXTENSION);
                            $safeAdditionalImageName = $fileNameWithoutExt_add . "_" . $counter_add . "." . $fileExt_add;
                            $additionalImageTargetFile = $productImagesDir . $safeAdditionalImageName;
                        }

                        if (move_uploaded_file($_FILES["images"]["tmp_name"][$i], $additionalImageTargetFile)) {
                            $additionalImagePaths[] = $additionalImageTargetFile;
                        } else {
                            error_log("Error uploading additional image: " . $additionalImageBasename);
                        }
                    }
                }
            }
            ...
            // Assign selected options to product and set their value states
                ...
                // Save custom wood/mould prices and selections
                $currentProductId = $productId; // $productId from lastInsertId()
                $productCustomData = [
                    'wood_option_selected' => isset($_POST['wood_option_checkbox']),
                    'selected_wood_sizes' => [],
                    'wood_prices' => [],
                    'mould_option_selected' => isset($_POST['mould_option_checkbox']),
                    'selected_mould_sizes' => [],
                    'mould_prices' => []
                ];

                if ($productCustomData['wood_option_selected'] && isset($_POST['wood_sizes']) && is_array($_POST['wood_sizes'])) {
                    $productCustomData['selected_wood_sizes'] = $_POST['wood_sizes'];
                    if (isset($_POST['wood_price']) && is_array($_POST['wood_price'])) {
                        foreach ($_POST['wood_sizes'] as $selectedSize) {
                            if (isset($_POST['wood_price'][$selectedSize]) && trim($_POST['wood_price'][$selectedSize]) !== '') {
                                $productCustomData['wood_prices'][$selectedSize] = $_POST['wood_price'][$selectedSize];
                            }
                        }
                    }
                }

                if ($productCustomData['mould_option_selected'] && isset($_POST['mould_sizes']) && is_array($_POST['mould_sizes'])) {
                    $productCustomData['selected_mould_sizes'] = $_POST['mould_sizes'];
                    if (isset($_POST['mould_price']) && is_array($_POST['mould_price'])) {
                        foreach ($_POST['mould_sizes'] as $selectedSize) {
                             if (isset($_POST['mould_price'][$selectedSize]) && trim($_POST['mould_price'][$selectedSize]) !== '') {
                                $productCustomData['mould_prices'][$selectedSize] = $_POST['mould_price'][$selectedSize];
                            }
                        }
                    }
                }
                ...
                if (file_exists($customPricesFilePath)) {
                    ...
                    $allCustomPricesData = json_decode($jsonContent, true);
                    if (json_last_error() !== JSON_ERROR_NONE) {
                        $allCustomPricesData = [];
                        error_log("Error decoding product_custom_prices.json before write (add product): " . json_last_error_msg());
                    }
                }
                $allCustomPricesData['product_' . $currentProductId] = $productCustomData;
                if (file_put_contents($customPricesFilePath, json_encode($allCustomPricesData, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES), LOCK_EX) === false) {
                    error_log("Error writing to product_custom_prices.json (add product)");
                    // Optionally append to $error or $success message for admin feedback
                }

            // $success = "Product added successfully!";
            // Update success message based on what was processed
            if (!empty($additionalImagePaths)) {
                // $success = "Product added successfully with main image and " . count($additionalImagePaths) . " additional image(s)!";
                $_SESSION['form_success_message'] = "Product added successfully with main image and " . count($additionalImagePaths) . " additional image(s)!";
            } else {
                // $success = "Product added successfully with main image!";
                $_SESSION['form_success_message'] = "Product added successfully with main image!";
            }
            header('Location: admin.php#products');
            exit();
            // If $uploadOk became 0 due to directory creation failure for additional images, $error would be set
            // If $mainImagePath is null, the whole block is skipped and $error (e.g. "Product image is required") should be shown.

        } elseif (!$mainImagePath && empty($error)) { // If mainImagePath is still null and no specific error was set before
             $error = "Sorry, there was an error processing the main product image.";
        }
        // If $error was set at any point, it will be displayed.
    } elseif (isset($_POST['delete_product'])) {
        $stmt = $pdo->prepare("DELETE FROM products WHERE id = ?");
        $stmt->execute([$_POST['product_id']]);
        $success = "Product deleted successfully!"; // This is not part of the add_product action, so leave as is or handle with PRG separately if needed.
        ...
        $imagePath = $_POST['current_image_path']; // Keep current main image if not updated
        $uploadOk = 1; // Assume okay initially
        ...
        if (isset($_FILES['images']) && isset($_FILES['images']['name'][0]) && !empty($_FILES['images']['name'][0])) {
            ...
            $targetDir = "uploads/"; // Main images go here

            // Process the new main image (first file in the array)
            $mainImageBasename = basename($_FILES["images"]["name"][0]);
            $safeMainImageName = preg_replace("/[^a-zA-Z0-9._-]/", "_", $mainImageBasename);

            if (empty($safeMainImageName) || $safeMainImageName === '.' || $safeMainImageName === '..') {
                $error = "Invalid new main image filename after sanitization.";
                $uploadOk = 0;
            } else {
                $mainImageTargetFile = $targetDir . $safeMainImageName;
                $mainImageFileType = strtolower(pathinfo($mainImageTargetFile, PATHINFO_EXTENSION));
                ...
                $check = getimagesize($_FILES["images"]["tmp_name"][0]);
                if ($check === false) {
                    $error = "New main file is not an image.";
                    $uploadOk = 0;
                }
                if ($_FILES["images"]["size"][0] > 9000000) { // 9MB
                    $error = "Sorry, your new main image file is too large.";
                    $uploadOk = 0;
                }
                if (!in_array($mainImageFileType, ["jpg", "jpeg", "png", "gif", "webp"])) {
                    $error = "Sorry, only JPG, JPEG, PNG, GIF & WEBP files are allowed for the new main image.";
                    $uploadOk = 0;
                }
                ...
                if ($uploadOk == 1) {
                    ...
                    if (move_uploaded_file($_FILES["images"]["tmp_name"][0], $mainImageTargetFile)) {
                        $imagePath = $mainImageTargetFile; // Update to new main image path
                        // Old main image is not deleted from uploads/ here, which is acceptable.
                    } else {
                        $error = "Sorry, there was an error uploading your new main image.";
                        $uploadOk = 0;
                    }
                }
            }
        } else {
            // No new files uploaded, keep existing $imagePath from current_image_path hidden input
            // If current_image_path was empty (e.g. product had no image), fetch it.
            if (empty($imagePath)) {
        ...
        if ($uploadOk == 1) { // Proceed if main image processing (if any) was okay
            // Update product details (including potentially new main image_path)
            ...
            $stmt->execute([
                ...
                $_POST['description'],
                $_POST['price'],
                $imagePath, // This is either new main image or existing one
                $_POST['category_id'],
                $productIdToUpdate
            ]);

            // Handle additional images IF new images were uploaded
            if ($newImagesUploaded) {
                $productImagesDir = "uploads/products/" . $productIdToUpdate . "/";
                if (!is_dir($productImagesDir)) {
                    if (!mkdir($productImagesDir, 0777, true)) {
                        $error = "Failed to create product image directory for additional images. Please check permissions.";
                        $uploadOk = 0; // Critical error for additional images
                    }
                }

                if ($uploadOk == 1) {
                    // Strategy: Replace All. Delete existing additional images first.
                    $existingAdditionalFiles = glob($productImagesDir . '*');
                    foreach ($existingAdditionalFiles as $file) {
                        if (is_file($file)) {
                            unlink($file);
                        }
                    }

                    // Process new additional images (from index 1 onwards)
                    $additionalImagePaths = [];
                    if (count($_FILES["images"]["tmp_name"]) > 1) {
                        for ($i = 1; $i < count($_FILES["images"]["tmp_name"]); $i++) {
                            if (isset($_FILES["images"]["name"][$i]) && $_FILES["images"]["error"][$i] == UPLOAD_ERR_OK) {
                                ...
                                if (empty($safeAdditionalImageName) || $safeAdditionalImageName === '.' || $safeAdditionalImageName === '..') {
                                    error_log("Update: Skipping additional image with invalid filename: " . $additionalImageBasename);
                                    continue;
                                }
                                ...
                                $check = getimagesize($_FILES["images"]["tmp_name"][$i]);
                                if ($check === false) {
                                    error_log("Update: Skipping additional file (not an image): " . $additionalImageBasename);
                                    continue;
                                }
                                if ($_FILES["images"]["size"][$i] > 9000000) {
                                    error_log("Update: Skipping additional image (too large): " . $additionalImageBasename);
                                    continue;
                                }
                                if (!in_array($additionalImageFileType, ["jpg", "jpeg", "png", "gif", "webp"])) {
                                    error_log("Update: Skipping additional image (invalid type): " . $additionalImageBasename);
                                    continue;
                                }

                                // Prevent overwriting by appending number
                                $counter_add = 0;
                                $originalAddTargetFile = $additionalImageTargetFile;
                                $originalSafeAddImageName = $safeAdditionalImageName;
                                while (file_exists($additionalImageTargetFile)) {
                                ...
                                if (move_uploaded_file($_FILES["images"]["tmp_name"][$i], $additionalImageTargetFile)) {
                                    ...
                                    error_log("Update: Error uploading additional image: " . $additionalImageBasename);
                                }
                            }
                        }
                    }
                     if (isset($error) && $uploadOk == 0) { // If directory creation failed for additional images
                        // Error message is already set
                    } elseif (!empty($additionalImagePaths)) {
                        $success = "Product updated successfully! New main image and " . count($additionalImagePaths) . " additional image(s) uploaded.";
                    } elseif ($newImagesUploaded && empty($additionalImagePaths) && count($_FILES["images"]["tmp_name"]) > 1) {
                        $success = "Product updated successfully! New main image uploaded. No new additional images were processed (check logs if some were expected).";
                    } else if ($newImagesUploaded) {
                         $success = "Product updated successfully! New main image uploaded.";
                    } else {
                         $success = "Product updated successfully! (No new images uploaded)";
                    }
                }
            } else if (!$newImagesUploaded) { // No new images uploaded at all
                 $success = "Product updated successfully! (Existing images retained)";
            }
            // If $uploadOk is 0 from main image processing, $error will be set and this block won't fully execute or success won't be set here.

            // Save custom wood/mould prices and selections for updated product
            $currentProductId = $productIdToUpdate; // $productIdToUpdate from $_POST['product_id']
            $productCustomData = [
                'wood_option_selected' => isset($_POST['wood_option_checkbox']),
                'selected_wood_sizes' => [],
                'wood_prices' => [],
            ...
            if (file_exists($customPricesFilePath)) {
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    $allCustomPricesData = [];
                    error_log("Error decoding product_custom_prices.json before write (update product): " . json_last_error_msg());
                }
            }
            $allCustomPricesData['product_' . $currentProductId] = $productCustomData;
            if (file_put_contents($customPricesFilePath, json_encode($allCustomPricesData, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES), LOCK_EX) === false) {
                error_log("Error writing to product_custom_prices.json (update product)");
                // Optionally append to $error or $success message for admin feedback
            }
        }
    }
...
if (file_exists($customPricesFilePath)) {
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        $allCustomPrices = []; // Reset if JSON is invalid
        error_log("Error decoding product_custom_prices.json: " . json_last_error_msg());
        // Optionally, set an admin-facing error message if needed
        // $error = "Warning: Could not load custom prices. JSON file is corrupted.";
    }
}
        ...
        <div class="row">
            ...
            <div class="col-md-10 p-4">
                ...
                <div class="tab-content">
                    ...
                    $productCustomPrices = []; // Initialize for the current product
                    if ($productToEdit && isset($allCustomPrices['product_' . $productIdToEdit])) {
                        $productCustomPrices = $allCustomPrices['product_' . $productIdToEdit];
                    }
                    ...
                    <div class="tab-pane <?= $editMode ? 'active' : '' ?>" id="add-product">
                        ...
                        <form method="post" enctype="multipart/form-data" action="admin.php<?= $editMode ? '?action=edit_product&product_id='.$productIdToEdit : '' ?>">
                            ...
                            <div class="mb-3">
                                <label for="image" class="form-label">Product Images <?= $editMode ? '(select one or more to update)' : '(select one or more for main image and gallery)' ?></label>
                                <input type="file" class="form-control" id="image" name="images[]" accept="image/*" multiple <?= $editMode ? '' : 'required' ?>>
                                <small class="form-text text-muted">
                                    <?= $editMode ? 'The first selected image will be the new main product image. Uploading new images will replace all existing additional images. If no new images are selected, current images will be retained.' : 'The first image will be the main product image. Subsequent images will be additional gallery images.' ?>
                                </small>
                            ...
                            <div class="mb-3">
                                ...
                                <?php endforeach; ?>
                                <!-- Wood Option Static HTML -->
                                <div class="mb-3">
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" name="wood_option_checkbox" id="wood_option_checkbox" value="wood" onchange="toggleHardcodedOptionValues(this, 'wood_sizes_values_div')"
                                            <?php if ($editMode && isset($productCustomPrices['wood_option_selected']) && $productCustomPrices['wood_option_selected']) echo 'checked'; ?>
                                        >
                                        <label class="form-check-label" for="wood_option_checkbox">
                                            <strong>Wood</strong>
                                        </label>
                                    </div>
                                    <div id="wood_sizes_values_div" class="ms-4 mb-3" style="display: <?php echo ($editMode && isset($productCustomPrices['wood_option_selected']) && $productCustomPrices['wood_option_selected']) ? 'block' : 'none'; ?>;">
                                        <small class="form-text text-muted">Select wood sizes:</small>
                                        <?php foreach ($woodSizes as $size):
                                            $currentWoodPrice = '';
                                            if ($editMode && isset($productCustomPrices['wood_prices']) && isset($productCustomPrices['wood_prices'][$size['value']])) {
                                                $currentWoodPrice = htmlspecialchars($productCustomPrices['wood_prices'][$size['value']]);
                                            }
                                        ?>
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="wood_sizes[]" id="wood_size_<?= str_replace(' ', '_', $size['value']) ?>" value="<?= htmlspecialchars($size['value']) ?>"
                                                    <?php
                                                    if ($editMode && isset($productCustomPrices['selected_wood_sizes']) && is_array($productCustomPrices['selected_wood_sizes']) && in_array($size['value'], $productCustomPrices['selected_wood_sizes'])) {
                                                        echo 'checked';
                                                    }
                                                    ?>
                                                >
                                                <label class="form-check-label" for="wood_size_<?= str_replace(' ', '_', $size['value']) ?>" style="margin-right: 10px;">
                                                    <?= htmlspecialchars($size['text']) ?>
                                                </label>
                                                <?php if ($size['value'] === "Customized size"): ?>
                                                    <input type="text" name="wood_price[<?= htmlspecialchars($size['value']) ?>]" value="<?= $currentWoodPrice ?>" placeholder="Price / Text" class="form-control form-control-sm" style="display: inline-block; width: auto; margin-left: 10px;">
                                                <?php else: ?>
                                                    <input type="number" step="0.01" name="wood_price[<?= htmlspecialchars($size['value']) ?>]" value="<?= $currentWoodPrice ?>" placeholder="Price" class="form-control form-control-sm" style="display: inline-block; width: auto; margin-left: 10px;">
                                                <?php endif; ?>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
                                <!-- End Wood Option Static HTML -->
                                <!-- Mould Option Static HTML -->
                                <div class="mb-3">
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" name="mould_option_checkbox" id="mould_option_checkbox" value="mould" onchange="toggleHardcodedOptionValues(this, 'mould_sizes_values_div')"
                                            <?php if ($editMode && isset($productCustomPrices['mould_option_selected']) && $productCustomPrices['mould_option_selected']) echo 'checked'; ?>
                                        >
                                        <label class="form-check-label" for="mould_option_checkbox">
                                            <strong>Mould</strong>
                                        </label>
                                    </div>
                                    <div id="mould_sizes_values_div" class="ms-4 mb-3" style="display: <?php echo ($editMode && isset($productCustomPrices['mould_option_selected']) && $productCustomPrices['mould_option_selected']) ? 'block' : 'none'; ?>;">
                                        <small class="form-text text-muted">Select mould sizes:</small>
                                        <?php foreach ($mouldSizes as $size):
                                            $currentMouldPrice = '';
                                            if ($editMode && isset($productCustomPrices['mould_prices']) && isset($productCustomPrices['mould_prices'][$size['value']])) {
                                                $currentMouldPrice = htmlspecialchars($productCustomPrices['mould_prices'][$size['value']]);
                                            }
                                        ?>
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="mould_sizes[]" id="mould_size_<?= str_replace(' ', '_', $size['value']) ?>" value="<?= htmlspecialchars($size['value']) ?>"
                                                    <?php
                                                    if ($editMode && isset($productCustomPrices['selected_mould_sizes']) && is_array($productCustomPrices['selected_mould_sizes']) && in_array($size['value'], $productCustomPrices['selected_mould_sizes'])) {
                                                        echo 'checked';
                                                    }
                                                    ?>
                                                >
                                                <label class="form-check-label" for="mould_size_<?= str_replace(' ', '_', $size['value']) ?>" style="margin-right: 10px;">
                                                    <?= htmlspecialchars($size['text']) ?>
                                                </label>
                                                <input type="number" step="0.01" name="mould_price[<?= htmlspecialchars($size['value']) ?>]" value="<?= $currentMouldPrice ?>" placeholder="Price" class="form-control form-control-sm" style="display: inline-block; width: auto; margin-left: 10px;">
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
                                <!-- End Mould Option Static HTML -->
                            </div>
                    ...
                    <script>
                        ...
                        function toggleHardcodedOptionValues(checkboxElement, valuesDivId) {
                            const valuesDiv = document.getElementById(valuesDivId);
